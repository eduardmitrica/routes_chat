...
- Activity. While people share it, the server sees when someone types in a chat or group (`chats/{chatId}/typing/{uid}`, `groups/{groupId}/typing/{uid}`), when their app is on screen (`presence/{uid}`), and which message they have read up to and when (`…/reads/{uid}`), never what they type or read. Which chats are unread is kept only on the phone, encrypted like drafts. Push notifications carry the sender's name and the chat or group id, never message text or a group's name; being added to a group is announced with the name of whoever added the person.
