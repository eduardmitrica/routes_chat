...
class MessagesWatcherBloc
  ...
  MessagesWatcherBloc(this._messageRepository)
    ...
    on<MessagesWatcherEvent>((event, emit) {
      switch (event) {
        case MessagesWatchAllStartedForChatWithId():
          ...
          _messagesSubscription = _messageRepository
              ...
              .listen(
                (failureOrMessage) => add(
                  MessagesWatcherEvent.messagesReceived(failureOrMessage),
                ),
              );
        case MessagesReceived():
          emit(
            event.failureOrMessages.fold(
              (failure) => MessagesWatcherState.loadFailure(failure),
              (messages) => MessagesWatcherState.loadSuccess(messages),
            ),
          );
      }
    });
