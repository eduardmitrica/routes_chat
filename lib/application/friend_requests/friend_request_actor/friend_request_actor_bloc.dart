...
class FriendRequestActorBloc
  ...
  FriendRequestActorBloc(this._friendRequestRepository, this._userRepository)
      : super(const FriendRequestActorState.initial()) {
    on<FriendRequestActorEvent>(
      (event, emit) async {
        await event.map(usernameChanged: (event) {
          ...
          if (username.isValid()) {
            final currentUserDetails = getIt<CurrentUseInformationPersistent>();
            ...
            if (sendingUserUsername != username.getOrCrash()) {
              ...
              await receivingUser.fold(
                ...
                (receivingUser) async {
                  ...
                  final searchResult =
                      await _friendRequestRepository.findBySenderAndReceiverIds(
                          friendRequest.senderId, friendRequest.receiverId);
                  await searchResult.fold((failure) async {
                    if (failure case NotFound()) {
                      final reverseSearchResult = await _friendRequestRepository
                      ...
                      await reverseSearchResult.fold((failure) async {
                        ...
                        emit(const FriendRequestActorState
                    ...
                    if (status case Pending()) {
                      emit(const FriendRequestActorState.requestAlreadySent());
                    } else {
